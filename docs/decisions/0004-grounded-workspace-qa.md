...
- A failed question can be explicitly retried by its owner through HTML or JSON. The retry atomically replaces the terminal failure with one new pending job claim; non-failed questions are not retryable.
...
| `POST /workspaces/:workspace_id/questions/:question_id/retry` | Requeues an owned failed question and returns `202 Accepted` for JSON. |
...
| Retry of a non-failed question | JSON returns `422 Unprocessable Entity` with `not_retryable`. |
